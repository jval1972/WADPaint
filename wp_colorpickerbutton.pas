...
type
  ...
  published
    ...
    property DropDownWidth: integer read FDropDownWidth write SetDropDownWidth default 14;
    property Enabled;
    ...
    property Color: TColor read FSelectionColor write SetSelectionColor default clBlack;
    property ShowHint;
...
begin
  ...
  if Result <> -1 then
    Exit;
  if (FOriginal.Width or FOriginal.Height) = 0
    then Exit;
...
procedure TColorPopup.WMRButtonDown(var Message: TWMRButtonDown);
begin
  Inherited;
    ...
    begin
      ...
      begin
        if FSelectedIndex < DefaultColorCount then
          Color := TColor(DefaultColors[FSelectedIndex].Color)
        else
          Color := TColor(SysColors[FSelectedIndex - DefaultColorCount].Color)
      end
      else if FSelectedIndex = CustomCell then
      begin
        if FCustomIndex < 0 then
          Color := FBWCombs[-(FCustomIndex + 1)].Color
        else if FCustomIndex > 0 then
          Color := FColorCombs[FCustomIndex - 1].Color;
      end
      else
        DoDefaultEvent;
    end;
...
function TColorPickerButton.MouseInButton: boolean;
var
  p: TPoint;
  r: TRect;
begin
...
begin
  ...
  begin
    if (FState in [bsDown, bsExclusive]) or
       (FMouseInControl and (FState <> bsDisabled) and MouseInButton) or
...
begin
  ...
  if Sender is TCustomAction then
    ...
    begin
      ...
      if Glyph.Empty and Assigned(ActionList) and Assigned(ActionList.Images) and
